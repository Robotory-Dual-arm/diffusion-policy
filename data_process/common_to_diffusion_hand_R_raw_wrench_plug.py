...
def main():
    input_filenames = ['/home/baetae/Downloads/common_data_plug.hdf5']
    output_filename = '/data/baetae/260430/diffusion_data_plug_raw_wrench_R_image.hdf5'
    output_demo_idx = 0
        ...
        for input_filename in input_filenames:
            with h5py.File(input_filename, 'r') as input_file:
                ...
                for demo_idx in tqdm.tqdm(range(demo_len), desc=f"Processing file"):
                    ...
                    # 각 robot timestamp에 가장 가까운 wrench 단일값을 매칭
                    ...
                    output_wrench_wrist_R_aligned = output_wrench_wrist_R[nearest_wrench_indices]
                    output_wrench_thumb_R_aligned = output_wrench_thumb_R[nearest_wrench_indices]
                    output_wrench_index_R_aligned = output_wrench_index_R[nearest_wrench_indices]
                    output_wrench_middle_R_aligned = output_wrench_middle_R[nearest_wrench_indices]
                    output_wrench_ring_R_aligned = output_wrench_ring_R[nearest_wrench_indices]
                    output_wrench_baby_R_aligned = output_wrench_baby_R[nearest_wrench_indices]
                    ...
                    output_obs.create_dataset('wrench_index_R', data=output_wrench_index_R_aligned[:-1])
                    output_obs.create_dataset('wrench_middle_R', data=output_wrench_middle_R_aligned[:-1])
                    output_obs.create_dataset('wrench_ring_R', data=output_wrench_ring_R_aligned[:-1])
                    output_obs.create_dataset('wrench_baby_R', data=output_wrench_baby_R_aligned[:-1])

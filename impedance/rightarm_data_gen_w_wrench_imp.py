...
def init_buffer():   
    return {
        'observations': {
            ...
            'desired_pose': [],    # impedance controller desired pose
...
def as_numeric_array(value, shape):
    if value is None:
...
DATA_SOURCE_BY_NAME = {
    'joint_R': '/dsr01/joint_states position',
    'hand_R': '/joint_states position',
    'wrench_wrist_R': '/aft_sensor2/wrench',
    'wrench_thumb_R': '/right_ft_sensor_broadcaster/wrench thumb',
    'wrench_index_R': '/right_ft_sensor_broadcaster/wrench index',
    'wrench_middle_R': '/right_ft_sensor_broadcaster/wrench middle',
    'wrench_ring_R': '/right_ft_sensor_broadcaster/wrench ring',
    'wrench_baby_R': '/right_ft_sensor_broadcaster/wrench baby',
}


def get_invalid_sample_summary(values):
    if len(values) == 0:
        return {
            'total': 0,
            'invalid': 0,
    ...
    if arr.ndim == 1:
        ...
        sample_axes = tuple(range(1, arr.ndim))
        invalid_mask = ~np.isfinite(arr).all(axis=sample_axes)
        all_nan_mask = np.isnan(arr).all(axis=sample_axes)

    invalid_indices = np.flatnonzero(invalid_mask)
    all_nan_count = int(all_nan_mask.sum())
    invalid_count = int(invalid_mask.sum())

    return {
        'total': len(values),
        'invalid': invalid_count,
        'all_nan': all_nan_count,
        'partial_nan': invalid_count - all_nan_count,
        'first_bad': int(invalid_indices[0]) if len(invalid_indices) > 0 else None,
        'reason': None,
    }


def print_data_availability_report(buffer):
    robot_steps = len(buffer['observations']['joint_R'])
    wrench_steps = len(buffer['observations']['wrench_wrist_R'])
    print(f"Data availability check: {robot_steps} robot steps, {wrench_steps} wrench steps")

    has_issue = False
    for name, source in DATA_SOURCE_BY_NAME.items():
        summary = get_invalid_sample_summary(buffer['observations'][name])
        if summary['invalid'] == 0 and summary['reason'] is None:
            continue

        has_issue = True
        if summary['reason'] is not None:
            print(f"  MISSING {name}: {summary['reason']} ({source})")
        else:
            print(
                f"  MISSING {name}: {summary['invalid']}/{summary['total']} bad samples "
                f"(all-NaN={summary['all_nan']}, partial-NaN={summary['partial_nan']}, "
                f"first_bad_idx={summary['first_bad']}) source={source}"
            )

    if not has_issue:
        print("  OK: all required numeric streams are present.")

    return not has_issue

def save_wrench_data(buffer, last_save_time):
    # Right arm wrench data
    buffer['observations']['wrench_wrist_R'].append(as_numeric_array(latest_wrench_aft_R, (6,)))
    buffer['observations']['wrench_thumb_R'].append(as_numeric_array(latest_wrench_thumb_R, (6,)))
    buffer['observations']['wrench_index_R'].append(as_numeric_array(latest_wrench_index_R, (6,)))
    buffer['observations']['wrench_middle_R'].append(as_numeric_array(latest_wrench_middle_R, (6,)))
    buffer['observations']['wrench_ring_R'].append(as_numeric_array(latest_wrench_ring_R, (6,)))
    buffer['observations']['wrench_baby_R'].append(as_numeric_array(latest_wrench_baby_R, (6,)))
...
def save_robot_data(buffer, images, last_save_time):
    ...
    # Joint positions
    buffer['observations']['joint_R'].append(as_numeric_array(latest_joint_R, (6,)))

    # Hand positions
    buffer['observations']['hand_R'].append(as_numeric_array(latest_hand_R, (15,)))

    # Desired pose (virtual target from impedance controller) @ 20Hz with robot/image
    buffer['observations']['desired_pose'].append(
        latest_desired_pose.copy() if latest_desired_pose is not None else None
    )

    # Images
    # buffer['observations']['image_H'].append(images[0].copy())
    buffer['observations']['image_T'].append(images[0].copy())
    buffer['observations']['image_R'].append(images[1].copy())
...
class JointSubscriber(Node):
    def __init__(self):
        ...
        self.hand_name = [f"right_thumb_joint{i}" for i in range(1,4)] + \
        ...
        self.warned_joint_name_fallback = False
        self.warned_hand_name_missing = False
    ...
    def joint_callback(self, msg):
        ...
        if any(v is None for v in joint_position) and len(msg.position) >= 6:
            if not self.warned_joint_name_fallback:
                print(f"Warning: /dsr01/joint_states does not contain {self.joint_name}. Using first 6 positions. msg.name={list(msg.name)}")
                self.warned_joint_name_fallback = True
            joint_position = list(msg.position[:6])

        latest_joint_R = as_numeric_array(joint_position, (6,))

    def hand_callback(self, msg):
        global latest_hand_R

        joint_mapping = {n: p for n, p in zip(msg.name, msg.position)}
        hand_position = [joint_mapping.get(j) for j in self.hand_name]   # thumb(3), index(3), middle(3), ring(3), baby(3)
        if any(v is None for v in hand_position) and not self.warned_hand_name_missing:
            print(f"Warning: /joint_states does not contain all hand joints {self.hand_name}. Missing values will be saved as NaN.")
            self.warned_hand_name_missing = True

        latest_hand_R = as_numeric_array(hand_position, (15,))
    ...
    def desired_pose_callback(self, msg):
        """Callback for impedance controller desired pose"""
        global latest_desired_pose
        latest_desired_pose = np.array(msg.data)
...
def main(args=None):
    ...
    serials = [ '151222078010', '126122270712']   #  Table, Right

    ### 카메라 Exposure 설정
    # exposures = [40, 50, 15000] # Head, Table, Right
    exposures = [40, 50, 15000] # Head, Table, Right
    ...
    FORCE_VIZ_SERIAL = '126122270712'

    # Find the index of the camera with the target serial number
    if ENABLE_FORCE_VISUALIZATION:
    ...
    elif not ENABLE_FORCE_VISUALIZATION:
        print("Force visualization disabled.")
    else:
    ...
    # 저장 주기
    # - force/wrench: 250Hz
    # - robot state + image + virtual target(desired_pose): 20Hz
    robot_hz = 20
    ...
    try: 
        while rclpy.ok():
            ...
            if ENABLE_FORCE_VISUALIZATION:
                ...
                force_viz_joints = None
                force_viz_vector = None
                force_finger_bar = None
            ...
            if recording: # data recording (250Hz)
                ...
                # Robot/Image/Desired pose는 20Hz로 저장
                ...
                while True:
                    ...
                    if data_store == 'y':
                        data_ok = print_data_availability_report(buffer)
                        if not data_ok:
                            save_bad_data = input("Missing data exists. Save anyway? (y/n): ").strip().lower()
                            if save_bad_data != 'y':
                                print("Demo not saved because missing data was detected. Press 's' to start a new recording.")
                                break

...
{
public:
    VRJointCommandNode() : Node("VRJointCommandSubscriber")
    {
        subscription_ = this->create_subscription<sensor_msgs::msg::JointState>(
            // "/joint_state_command",  // Topic name
            "left_dsr_joint_controller/joint_state_command",  // Topic name
            10,
            std::bind(&VRJointCommandNode::joint_command_callback, this, std::placeholders::_1)
        );
        RCLCPP_INFO(this->get_logger(), "VR Joint Command Subscriber initialized on left_dsr_joint_controller/joint_state_command");
    }
...
{
public:
    TaskSpaceCommandNode() : Node("TaskSpaceCommandSubscriber")
    {
        subscription_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
            "/left_dsr_controller/task_space_command",
            10,
...
{
public:
    CsvPoseCommandNode() : Node("CsvPoseCommandSubscriber")
    {
...
TorqueRtNode::TorqueRtNode() : Node("TorqueRt")
{
    ...
    filename << data_dir << "/impedance_control_data_vr_" 
...
void TorqueRtNode::TorqueRtStreamPublisher()
{
    ...
    if(first_get)
    {
        ...
        if(print_counter >= PRINT_INTERVAL)
        {
            ...
            printf("=== [VR MODE] Impedance Control Data ===\n");

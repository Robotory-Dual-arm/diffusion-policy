...
R_R = np.array([
    [1.0,      0.0,      0.0    ],   # 오른팔 x → 월드 +x
    [0.0,     -sqrt2,    sqrt2  ],   # 오른팔 y → 월드 (z-y)/√2
    [0.0,     -sqrt2,   -sqrt2  ],   # 오른팔 z → 월드 (-z-y)/√2
])

# Translation: 왼팔 베이스는 월드 +Y 0.5m, 오른팔은 월드 -Y 0.5m
T_L = np.array([0.0, 0.5, 0.0])  # 월드 좌표계에서 왼팔 베이스 위치
T_R = np.array([0.0, -0.5, 0.0]) # 월드 좌표계에서 오른팔 베이스 위치
